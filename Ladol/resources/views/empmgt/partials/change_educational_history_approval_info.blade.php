...
<div class="card card-shadow card-inverse white bg-facebook">
    <div class="card-block p-20 h-full">
        <h3 class="white m-t-0">{{$activity->causer?$activity->causer->name:''}} made changes to their Educational History</h3>
            ...
            @if($activity->changes!='')
                ...
                @foreach($activity->changes['attributes'] as  $key=>$change)
                    ...
                    @if($activity->description=='updated')
                        @if(($activity->changes['old'][$key]!=$activity->changes['attributes'][$key])&&$key!='qualification_id'&&$key!='emp_id'&&$key!='emp_id'&&$key!='company_id')
                            <ul class="list-group ">
                    ...
                    @else
                        @if($key!='qualification_id'&&$key!='emp_id'&&$key!='company_id')
                        <ul class="list-group ">
                            <li class=" list-group-item bg-grey-300 ">
                ...
                <form action="" id="approveChangeForm" method="POST">
                    ...
                    @php
                        $exemptions=['emp_id','last_change_approved_on','last_change_approved','last_change_approved_by','qualification.name','company_id'];
                    @endphp
                    ...
                    @foreach($activity->changes['attributes'] as  $key=>$change)
                        ...
                        @endif
                            @endforeach
                    @csrf
                    <input type="hidden" name="type" value="approve_educational_history_changes">
