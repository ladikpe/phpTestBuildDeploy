...
<!-- Page -->
  <div class="page ">
  	<div class="page-header">
  		<h1 class="page-title">{{__('Recruit')}} (External Jobs)</h1>
		  <ol class="breadcrumb">
      ...
      @forelse($joblistings as $joblisting)
      	<div class="panel panel-info panel-line panel-bordered">
            <div class="panel-heading">
              <h3 class="panel-title">{{$joblisting->title}}</h3>
            ...
            <div class="panel-body">
            	<div class="ribbon ribbon-clip ribbon-reverse ribbon-danger">
                      ...
                      <div class="ribbon ribbon-clip ribbon-primary">
                        <span class="ribbon-inner"><a href="{{url('recruits/view_ejob_listing').'?listing_id='.$joblisting->id}}" style="color: #fff;">View</a></span>
                      </div>
        ...
        <div class="col-md-4">
              ...
              <form class="" action="{{url('recruits/ejobs')}}" method="get" >
  ...
  @include('recruit.modals.addJoblisting')
   @include('recruit.modals.editEJoblisting')
...
<script src="{{asset('global/vendor/select2/select2.min.js')}}"></script>
 ...
 $('.enable_job').on('change', function() {
      listing_id= $(this).attr('id');
       ...
       $.get('{{ url('/recruits/change_elisting_status') }}/',{ listing_id: listing_id },function(data){
        if (data==1) {
 ...
 $('#target').on('change', function() {
    ...
    $(document).on('submit','#editEJobListingForm',function(event){
     event.preventDefault();
     var form = $(this);
        var formdata = false;
        if (window.FormData){
        ...
        $.ajax({
            ...
            success     : function(data, textStatus, jqXHR){
                ...
                toastr.success("Changes saved successfully",'Success');
               $('#editEJobListingModal').modal('toggle');
...
var state=$('#state').val();
    ...
    $.get('{{ url('/recruits/get_ejob_listing_info') }}/',{ listing_id: listing_id },function(data){
     
     $('#editjtitle').val(data.title);
      $('#editjjob_ref').val(data.job_ref);
       $('#editjdescription').val(data.description);
        $('#editjskills').val(data.skills);
         $('#editjexperience').val(data.experience);
         $('#editcountryid').val(data.country_id);
         $('#editstateid').val(data.state_id);
  ...
  function deleteJobListing(listing_id){
    $.get('{{ url('/recruits/delete_ejob_listing') }}/',{ listing_id: listing_id },function(data){
      if (data=='success') {
    toastr.success("Job Listing deleted successfully",'Success');
      }else{
        toastr.error("Error deleting Job Listing",'Success');
      }
