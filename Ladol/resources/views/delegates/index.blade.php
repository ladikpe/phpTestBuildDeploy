...
<!-- Page -->
  <div class="page ">
    <div class="page-header">
      <h1 class="page-title">{{__('Delegate Role')}}</h1>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{url('/')}}">{{__('Home')}}</a></li>
        <li class="breadcrumb-item active">{{__('Delegate Role')}}</li>
      </ol>
    ...
    <div class="panel panel-info panel-line">
      <div class="panel-heading">
        <h3 class="panel-title">Roles Delegated {{$delegates->count()}}
            ...
            title="Delegate Role" style="float:right" id="add"><i class="la la-plus"></i> Delegate Role</button>
        </h3>
          ...
          <tbody>
            ...
            @foreach($delegates as $delegate)
                ...
                <tr>           
                  ...
                  <td>{{$delegate->approval_request?$delegate->approval_request->user->name:''}} Requesting  
                    {{$delegate->approval_request?$delegate->approval_request->reason:''}}</td>
...
<div class="modal fade in modal-3d-flip-horizontal modal-info" id="addModal" aria-hidden="true" aria-labelledby="addModal" role="dialog" tabindex="-1">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-header" >
          ...
          <h4 class="modal-title" id="training_title">Role Delegate Form </h4>
        </div>
                ...
                <div class="col-sm-9">
                    <select class="form-control select2" name="approval_request_id" id="approval_request_id">
                        ...
                        @forelse($approval_requests as $approval_request)
                            <option value="{{$approval_request->id}}">{{$approval_request->user->name}}, &nbsp; &nbsp; &nbsp;  Requested for &nbsp; {{$approval_request->reason}}</option>
                        @empty
                        @endforelse
                    </select>
                </div>
