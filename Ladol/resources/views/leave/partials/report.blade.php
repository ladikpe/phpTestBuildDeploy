   <table class="table table striped">
         	<thead>
              ...
              <th>Designation</th>
         		...
         		{{--<th>With Pay</th>--}} {{--"Removed as per client request"--}}
         	</tr>
         	</thead>
         	<tbody>
          ...
          <tr>
            ...
            <td>{{date("F j, Y", strtotime($leave_request->user->hiredate))}}</td>
             <td>{{$leave_request->user->user_grade?$leave_request->user->user_grade->level:''}}</td>
            ...
            <td>{{$leave_request->user->job?$leave_request->user->job->title:''}}</td>
         	<td>{{$leave_request->leave_name}}</td>
         		...
         		{{--<td>{{$leave_request->paystatus==0?'without pay':'with pay'}}</td>--}} {{--"Removed as per client request"--}}
