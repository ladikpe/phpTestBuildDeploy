...
const getPensionFundAdmins = async (): Promise<{
  data: TPensionFundAdmin[];
  total: number;
}> => {
  const url = `/api/pension-fund-admins`;
  ...
  const data: TPensionFundAdmin[] = result.map(
    (item: TPensionFundAdmin): TPensionFundAdmin => ({
...
export const useFetchPensionFundAdmins = () => {
  const [state, setState] = useState<TState>("loading");
  const [total, setTotal] = useState<number>(0);
  const [data, setData] = useState<TPensionFundAdmin[]>([]);
  useEffect(() => {
    setState("loading");

    getPensionFundAdmins()
      .then((res) => {
        setData(() => res.data);
        setState(() => "success");
        setTotal(() => res.total);
