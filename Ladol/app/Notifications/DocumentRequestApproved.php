...
//'You are to review and approve a request for a document '.$this->document_request_approval->document_request->title.' applied for by '.$this->document_request->user->name
        ...
        ->line('The Document, '.$this->document_request_approval->document_request->title.' which you requested for  on the '.date('Y-m-d',strtotime($this->document_request_approval->document_request->created_at)).'('.\Carbon\Carbon::parse($this->document_request_approval->document_request->created_at)->diffForHumans().') has been approved at the final stage')
    ...
    public function toDatabase($notifiable)
    {
          ...
          return new DatabaseMessage([
              'subject'=>'Document Request Approved',
            'message'=>'The document request, '.$this->document_request_approval->document_request->title.' which you submitted for approval on the '.date('Y-m-d',strtotime($this->stage->workflow->name)). 'has been approved at the final stage.',
