...
@section('content')
                ...
                <div class="col-ms-12 col-xs-12 col-md-12">
                    <div class="panel panel-info panel-line">
                        <div class="panel-heading">
                            <h3 class="panel-title">My Document Request</h3>
                            <div class="panel-actions">

                                <button class="btn btn-info" data-toggle="modal" data-target="#addDocumentRequestModal">
                                    New
                                    Document Request
                                </button>
                                ...
                                <table class="table table striped">
                                    ...
                                    <tr>
                                        @foreach ($document_requests as $document_request)
                                            <td>{{ $document_request->document_request_type ? $document_request->document_request_type->name : '' }}
                                            </td>
                                            ...
                                            <td><span
                                                        class=" tag   {{ $document_request->status == 0 ? 'tag-warning' : ($document_request->status == 1 ? 'tag-success' : 'tag-danger') }}">{{ $document_request->status == 0 ? 'pending' : ($document_request->status == 1 ? 'approved' : 'rejected') }}</span>
                                            </td>
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="exampleIconDropdown1"
                                                        ...
                                                        @if ($document_request->file != '')
                                                            <a style="cursor:pointer;" class="dropdown-item" id=""
                                                               href="{{ url('document_requests/download?document_request_id=' . $document_request->id) }}"><i
    ...
    <!-- End Page -->
    @include('document_requests.modals.addrequest')
...
@section('scripts')
            ...
            $(function () {
                ...
                function doValidate(v) {
                    ...
                    if (check < 0) {
                        ...
                        toastr.error('Your leave days cannot exceed your entitled days (' + $(
                ...
                function viewRequestApproval(document_request_id) {
                    $(document).ready(function () {
                        $("#detailLoader").load(
                            '{{ url('/document_requests/get_details') }}?document_request_id=' +
                            document_request_id);
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            datePicker();
            $('#addDocumentRequestForm').submit(function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus, jqXHR) {
                        ...
                        toastr["success"]("Changes saved successfully", 'Success');
                        $('#addDocumentRequestModal').modal('toggle');
                        location.reload();
                    },
                     error: function (data, textStatus, jqXHR) {
                         jQuery.each(data['responseJSON'], function (i, val) {
            ...
            // script to show file upload field 
            $('#abtype').change(function () {
                if ($(this).val() !== "") {
                    $('#docUpload').show();
                } else {
                    $('#docUpload').hide();
