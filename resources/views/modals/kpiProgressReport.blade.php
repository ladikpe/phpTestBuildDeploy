...
@php $disable2=Auth::user()->role->permissions->contains('constant', 'edit_performance') && ($employee->id!=Auth::user()->id) ? '' : 'disabled';
     $disable=Auth::user()->role->permissions->contains('constant', 'edit_performance')==false || ($employee->id==Auth::user()->id) ? '' : 'disabled';
@endphp
      ...
      <div class="modal-body">
        <form id="addKpiReport">
         ...
         <div class="modal-footer">
          ...
          <button id="submitReport" {{Auth::user()->role->permissions->contains('constant', 'edit_performance') && $employee->id!=Auth::user()->id ? '' : 'disable' }} type="submit" class="btn btn-success"> Submit </button> 
        </form>
