...
<div class="card card-shadow card-inverse white bg-facebook">
    <div class="card-block p-20 h-full">
        <h3 class="white m-t-0">{{$activity->causer?$activity->causer->name:''}} made changes to their Employment History</h3>
            ...
            @if($activity->changes!='')
                ...
                @foreach($activity->changes['attributes'] as  $key=>$change)
                    ...
                    @if($activity->description=='updated')
                        @if(($activity->changes['old'][$key]!=$activity->changes['attributes'][$key])&&$key!='user_id'&&$key!='company_id')
                            <ul class="list-group ">
                ...
                <form action="" id="approveChangeForm" method="POST">
                    ...
                    @foreach($activity->changes['attributes'] as  $key=>$change)
                        ...
                        @endif
                            @endforeach
                    @csrf
                    <input type="hidden" name="type" value="approve_employment_history_changes">
